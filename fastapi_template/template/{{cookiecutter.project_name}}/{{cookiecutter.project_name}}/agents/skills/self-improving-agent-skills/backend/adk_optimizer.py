...
class SkillOptimizer:
    ...
    async def analyze_skill(self, skill_files: dict) -> dict:
        ...
        prompt = (
            ...
            f"Return JSON:\n"
            + (
                '{"scenarios": [{"id": 1, "name": "short name", "description": "short name", '
                '"input": "the user request to test"}], '
                '"evals": [{"id": 1, "name": "what to check", "criterion": "what to check", '
                '"question": "yes/no question about the output", '
                '"pass_condition": "what yes looks like", "fail_condition": "what no looks like"}]}'
            )
        )
    ...
    async def _score_skill(self, skill_md, scenarios, evals):
        ...
        for sc in scenarios:
            ...
            example_json = (
                '{"results": [{"eval_id": 1, "passed": true, "reason": "ok"}]}'
            )
